...
import { BarChart3, AlertCircle, CheckCircle2, ArrowRightLeft, MinusCircle } from "lucide-react";
...
export default function StoreCapacity() {
  ...
  const stats = { totalSkus: 1542, capacity: 1000, planned: 985, get remaining() { return this.capacity - this.planned }, notInPlan: 557, needsReview: 42 };
  ...
  const getSalesLabel = (sales: string) => {
    switch (sales) {
      case "高": return <div className="flex items-center text-emerald-600 font-bold">↑ 高</div>;
      case "中": return <div className="flex items-center text-blue-600 font-bold">→ 中</div>;
      case "低": return <div className="flex items-center text-rose-600 font-bold">↓ 低</div>;
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
          ...
          <span>/</span>
          <span className="text-foreground font-medium">キャパシティ管理</span>
        </div>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-6">
        <Card className="border-none shadow-md shadow-black/5 bg-secondary/20">
          <CardHeader className="pb-2"><CardTitle className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">総SKU数</CardTitle></CardHeader>
          <CardContent><div className="text-lg font-black">{stats.totalSkus.toLocaleString()}</div></CardContent>
        </Card>
        <Card className="border-none shadow-md shadow-black/5 bg-primary/5">
          <CardHeader className="pb-2"><CardTitle className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">売場キャパ数</CardTitle></CardHeader>
          <CardContent><div className="text-lg font-black">{stats.capacity.toLocaleString()}</div></CardContent>
        </Card>
        <Card className="border-none shadow-md shadow-black/5">
          <CardHeader className="pb-2"><CardTitle className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider">展開予定SKU数</CardTitle></CardHeader>
          <CardContent><div className="text-lg font-black">{stats.planned.toLocaleString()}</div></CardContent>
        </Card>
        <Card className="border-none shadow-md shadow-black/5 bg-emerald-50/50 dark:bg-emerald-950/10">
          <CardHeader className="pb-2"><CardTitle className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider">キャパ残</CardTitle></CardHeader>
          <CardContent><div className="text-lg font-black text-emerald-700">{stats.remaining.toLocaleString()}</div></CardContent>
        </Card>
        <Card className="border-none shadow-md shadow-black/5">
          <CardHeader className="pb-2"><CardTitle className="text-[10px] font-bold text-orange-600 uppercase tracking-wider">未展開SKU数</CardTitle></CardHeader>
          <CardContent><div className="text-xl font-black">{stats.notInPlan.toLocaleString()}</div></CardContent>
        </Card>
        <Card className="border-none shadow-md shadow-black/5 bg-rose-50 dark:bg-rose-950/20">
          <CardHeader className="pb-2"><CardTitle className="text-[10px] font-bold text-rose-600 uppercase tracking-wider">要見直しSKU数</CardTitle></CardHeader>
          <CardContent><div className="text-xl font-black text-rose-600">{stats.needsReview}</div></CardContent>
        </Card>
      </div>
              ...
              <TableBody>
                {MOCK_OPTIMIZATION_DATA.map((item) => (
                  <TableRow key={item.sku} className="hover:bg-secondary/5 transition-colors">
                    ...
                    <TableCell>
                      <Badge variant="outline" className="font-bold border-primary/20 text-primary">{item.block}</Badge>
                    </TableCell>
                    <TableCell><span className="text-sm">{item.status}</span></TableCell>
                    <TableCell>{getSalesLabel(item.sales)}</TableCell>
