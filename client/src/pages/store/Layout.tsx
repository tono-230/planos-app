...
import { useStorePlan, FIXTURES, BRAND_COLORS } from "@/context/StorePlanContext";
...
export default function StoreLayout() {
  ...
  const { assignments, selectedFixtureId, setSelectedFixtureId } = useStorePlan();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {FIXTURES.map(fixture => {
              ...
              return (
                <button
                  key={fixture.id}
                  style={fixture.style}
                  className={`absolute rounded-lg border-2 transition-all duration-200 flex flex-col items-center justify-center gap-1 cursor-pointer ${isSelected ? selStyle : baseStyle}`}
            ...
            <div className="rounded-2xl border border-border/60 bg-card shadow-sm overflow-hidden animate-in slide-in-from-right-4 duration-200">
              ...
              <div className="px-5 py-4 space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <CapacityBar used={selectedBrands.length * 8} total={selectedFixture.capacity} />
                </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {FIXTURES.map(fixture => {
            const isSelected = selectedFixtureId === fixture.id;
            const brands = assignments[fixture.id] ?? [];
            const pct = Math.round(Math.min((brands.length * 8 / fixture.capacity) * 100, 100));
            return (
              <button
                key={fixture.id}
                onClick={() => setSelectedFixtureId(isSelected ? null : fixture.id)}
      ...
      <p className="text-xs text-muted-foreground/60 italic">
        ※ レイアウトはモックデータです。什器を選択すると「今週の売場計画」ページとも連動します。
      </p>
    </div>
  );
}
